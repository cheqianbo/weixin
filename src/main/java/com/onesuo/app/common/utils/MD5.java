...
/**
 * ClassName:    MD5 算法的Java Bean
 * Description:  md5 类实现了RSA Data Security, Inc.在提交给IETF
 的RFC1321中的MD5 message-digest 算法。
 * Copyright:    Copyright@2005-2006
 * Company:      Statute@HANWEB
 * Version:      2.0
 * History:      2001-09-20  ssl  V2.0    创建
 *
 */
...
public class MD5 {
	...
	public String getKeyEd(String Txt, String encrypt_key) {// 用encrypt_key将明文打乱
	...
	public String setFromBASE64(String s) {
		...
		try {
			String b = encoder.encode(s.getBytes("UTF-8"));
			return b;
		} catch (Exception e) {
			return "";
		}
	...
	public String getTxtChang(String Txt) { // 分别颠倒基偶数，并连接成字符串
		StringBuffer temp1 = new StringBuffer();
		...
		for (int i = 0; i < Txt.length(); i++) {
			if (i % 2 == 0) {
				...
				temp1 = temp1.append(tem); // 取得字符串的奇数位字符,并组合成新的字符串 $tmp1
			} else {
				char tem1 = getByteDEX(Txt.charAt(i));
				temp2 = temp2.append(tem1); // 取得字符串的偶数位字符,并组合成新的字符串 $tmp2
			}
		}
	...
	public char getByteDEX(char ib) { // 把每个字符的ASCII码的二进制编码按规则:
		int num = (int) ib;
		String num1 = Integer.toBinaryString(num);
		StringBuffer temp1 = new StringBuffer();
		int n = num1.length();
